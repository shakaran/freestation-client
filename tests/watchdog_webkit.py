...
class Watchdog(object):
    ...
    def monitor(self, alive_time = None, enable_gui = True, executions = None):
        ...
        while(self.play):
            ...
            if os.path.exists('/proc/' + str(self.children_pid)):
                print 'Alive', self.children_pid
            else:
                print 'NO Alive', self.children_pid
            
            ##print self.p
            
            ##if self.p.is_alive() == False or self.p.finished: 
            ##    self.p.on_destroy(None, None)
